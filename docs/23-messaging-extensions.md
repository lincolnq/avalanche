...
- **No JS bridge — I/O is URL params in, deeplinks out.** The webview is a plain sandboxed page with *no* privileged JS API to the native app. Everything inbound (which Project, which group, entry params, a **scoped identity token** — zkgroup-style, proving "this user, in this group, with these granted scopes," see `20-project-security.md`) arrives as **query params on the launch URL**, exactly as the project-token already does. Everything outbound is an **intercepted deeplink** the page navigates to on the `go.theavalanche.net` host — `https://go.theavalanche.net/compose/attach?url=…` to return content, `…/close` to dismiss (and `…/conversation/<did>`, `…/i/<token>` for app navigation). These are caught by **host match** in the webview's navigation delegate (the canonical form — see `20-project-security.md`): the delegate cancels the HTTP navigation directly, so it does **not** rely on Universal Links / App Links firing (which are unreliable from inside an app's own webview). Emit `https://go.theavalanche.net/…` links, never a custom scheme. This is a strictly smaller native attack surface than a bridge, and it keeps `20-project-security.md`'s "no JS bridge" stance intact.
