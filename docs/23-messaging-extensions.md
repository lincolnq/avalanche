...
- **Return content you send.** The webview hands a result back through the bridge and *you* send it as a normal message/attachment. Giphy is this: pick a GIF → it's inserted into your message. The Project needs no bot in the conversation — it's a pure compose helper (service mode), the Telegram inline-bot pattern.
- **The bot posts the result.** The webview is a destination; the Project's bot (a conversation member) posts the outcome back — a survey tally, an RSVP summary. This is the cardstack.
...
- **Zero access to E2E data.** The webview never sees message history, the local store, or keys. It receives only a **launch context** through a minimal JS bridge: which Project, which group, entry params, a **scoped identity token** (zkgroup-style, proving "this user, in this group, with these granted scopes" — `20-project-security.md`), and — for the return-content mode — a "send this back to the composer" call. Opening a Project means **leaving the E2E conversation for that Project's server-backed trust domain** — fine because it's explicit and full-screen, but the chrome must signpost it.
- **The bridge is the security surface — keep it tiny** (launch context, scoped token, return-content, "post a result back," theming, close, deep-link handling). Model it on Telegram's `WebApp` API. Default answer to "can the webview do X" is no.
...
**Giphy / stickers** — a Project entry point in the "+" menu. Tapping it opens the Giphy webview; you search and pick; the webview returns the GIF through the bridge and *you* send it as a normal attachment. No bot, no conversation membership — a pure compose helper.
...
1. **Full-screen webview rendering & bridge spec.** The webview surface is arguably bigger than this doc — the bridge API (incl. return-content and post-result), the scoped-token format, isolation guarantees, content packaging (remote-loaded vs. signed bundle), and App Store constraints need their own pass, likely a dedicated `2x` doc, with a security review alongside `20-project-security.md`.
