...
- [ ] **T09** — `src/services/DevServerActnetService.ts`: implements `ActnetService` by calling `invoke('snake_case_command', args)` for every method. Add a matching stub `#[tauri::command]` in `src-tauri/src/lib.rs` for each (return `Err("not implemented".to_string())` — just enough to compile and register). Register all commands in the `tauri::Builder` call. **Also update `src-tauri/capabilities/default.json`** — add every new command name to the `"permissions"` array, or `invoke()` will fail at runtime.
